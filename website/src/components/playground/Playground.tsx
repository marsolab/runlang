...
function useCompiler() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e instanceof Error ? e.message : "Failed to load WASM");
      }
    })();
    return () => {
      cancelled = true;
  ...
  const callWasm = useCallback(
    (
      fn: keyof Pick<WasmExports, "check" | "tokenize" | "parse" | "format" | "run">,
      source: string,
    ): string | null => {
  ...
  return (
    <button
      ...
      className={`px-4 py-2 text-sm font-medium transition-colors ${
        active ? "border-b-2 border-blue-500 text-blue-400" : "text-gray-400 hover:text-gray-200"
      }`}
    >
      {label}
    </button>
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-run-code-bg px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          {!ready && <span className="text-xs text-gray-500">Loading compiler...</span>}
          <button
            type="button"
            onClick={handleRun}
            disabled={!ready}
          ...
          >
            Run
            <span className="ml-1.5 hidden text-xs text-blue-200 sm:inline">
              {isMac ? "\u2318" : "Ctrl"}+\u23CE
            </span>
          </button>
        </div>
      ...
      <div className="grid flex-1 grid-cols-1 md:grid-cols-2">
        ...
        <div className="relative min-h-[200px] border-r border-white/10">
          <div className="absolute left-0 top-0 px-4 py-2 text-xs text-gray-500">source.run</div>
          <textarea
            ref={textareaRef}
            value={code}
        ...
        <div className="relative min-h-[200px] overflow-auto bg-run-dark">
          <div className="absolute left-0 top-0 px-4 py-2 text-xs text-gray-500">output</div>
          <pre
